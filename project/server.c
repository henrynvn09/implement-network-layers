...
int main(int argc, char **argv)
{
  ...
  Receive_buffer receive_buffer[WINDOW_SIZE] = {0};
  ...
  while (1)
  {
    ...
    if (bytes_recvd > 0)
    {
      ...
      print_current_buffer(receive_buffer, &receive_l, &receive_r);
      print_data_buffer(receive_buffer, &receive_l, &receive_r, &client_seq);
